...
public class XMLTableTreeViewerEx extends TreeViewer implements IDesignViewer {
	
	class NodeActionMenuListener implements IMenuListener {
		public void menuAboutToShow(IMenuManager menuManager) {
			// used to disable NodeSelection listening while running
			// NodeAction
			XMLNodeActionManager nodeActionManager = new XMLNodeActionManager(((IDOMDocument) getInput()).getModel(), XMLTableTreeViewerEx.this) {
	...
	private class SelectionProvider implements IPostSelectionProvider {
		...
		public void addPostSelectionChangedListener(ISelectionChangedListener listener) {
			XMLTableTreeViewerEx.this.addPostSelectionChangedListener(listener);
		}

		public void removePostSelectionChangedListener(ISelectionChangedListener listener) {
			XMLTableTreeViewerEx.this.removePostSelectionChangedListener(listener);
		}

		public void addSelectionChangedListener(ISelectionChangedListener listener) {
			XMLTableTreeViewerEx.this.addSelectionChangedListener(listener);
		}

		public ISelection getSelection() {
			return XMLTableTreeViewerEx.this.getSelection();
		}

		public void removeSelectionChangedListener(ISelectionChangedListener listener) {
			XMLTableTreeViewerEx.this.removeSelectionChangedListener(listener);
		}
		...
		public void setSelection(ISelection selection) {
							...
							if (startStructuredDocumentRegion != null) {
								...
								if (matchingRegion != null && matchingRegion.getType().equals(DOMRegionContext.XML_TAG_ATTRIBUTE_NAME)) {
									...
									if (attributes != null && attrName.length() > 0) {
										...
										if (attr != null) {
											selectionSet = true;
											XMLTableTreeViewerEx.this.setSelection(new StructuredSelection(attr));
										}
									}
								}
							}
			...
			if (!selectionSet) {
				XMLTableTreeViewerEx.this.setSelection(selection);
			}
		}
	}
	...
	protected XMLTreeExtensionEx treeExtension;
	
	private ISelectionProvider fSelectionProvider = new SelectionProvider();
	public boolean justRenamed = false;

	public XMLTableTreeViewerEx(Composite parent) {
		super(parent, SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL);

		// set up providers
		this.treeExtension = new XMLTreeExtensionEx(getTree());

		XMLTableTreeContentProviderEx provider = new XMLTableTreeContentProviderEx();
			...
			public void drop(DropTargetEvent event) {
				if (event.operations != DND.DROP_NONE && LocalSelectionTransfer.getTransfer().getSelection() instanceof IStructuredSelection) {
					...
					if (selection != null && !selection.isEmpty() && event.item != null && event.item.getData() != null) {
						...
						final DragNodeCommand command = new DragNodeCommand(event.item.getData(), feedbackFloat, event.operations, event.detail, selection.toList(), XMLTableTreeViewerEx.this);
