				...
				Job job = new Job("Validating") {
					...
					protected IStatus run(IProgressMonitor monitor) {
						try {
							...
							System.out.println("字符串处理时间：" + (t2 - t1));
							is.close();
							long t3 = System.currentTimeMillis();
							System.out.println("关闭流:" + (t3 - t2));
							...
							getShell().getDisplay().asyncExec(new Runnable() {
								@Override
								public void run() {
									MessageDialog.openInformation(getActiveEditor().getSite().getShell(), "返回内容", "参考延时:" + delay + "\n" + sb.toString());
								}
							});
...
//							System.out.println(chkConnection);
							...
							getShell().getDisplay().asyncExec(new Runnable() {
								@Override
								public void run() {
									MessageDialog.openError(getActiveEditor().getSite().getShell(), "错误", "验证失败");
								}
							});
						} catch (IOException e) {
							getShell().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				Job job = new Job("Validating") {
					...
					protected IStatus run(IProgressMonitor monitor) {
						...
						monitor.beginTask("验证...", size);
						for (Proxy proxy : proxies) {
							validate(proxy);
						}
						System.out.println("全部验证完成！！！");
						monitor.done();
						return Status.OK_STATUS;
					}
...
//				final boolean[] timeout = new boolean[] {false};
				...
				Thread thread = new Thread("xx") {
					...
					public void run() {
						try {
							...
							if (sb.indexOf("百度一下") != -1) {
								proxy.setLastValidateDate(new Date());
								proxy.setDelay((int) delay);
								System.out.println("代理" + proxy.getName() + "验证成功，参考延时:" + delay);
								validteOk[0] = true;
								return;
							} 
							
						} catch (MalformedURLException e) {
							//System.out.println("代理" + proxy.getName() + "验证失败！");
						} catch (IOException e) {
							//System.out.println("代理" + proxy.getName() + "验证失败！");
						}
					}
				...
				if (!validteOk[0]) {
					thread.interrupt();
					System.out.println("代理" + proxy.getName() + "验证失败！【超时】 ");
					proxy.setDelay(-1);
				}
